...
define( "WB_AKEY" , Configure::read('weibo.appid'));
define( "WB_SKEY" , Configure::read('weibo.appkey') );
...
function get_friends()
{
	$c = new SaeTClientV2( WB_AKEY , WB_SKEY , $_SESSION['weibo_token']['access_token'] );
	$uid = $_SESSION['weibo_token']['uid'];
	$friends_message = $c->friends_by_id( $uid);//获取当前登录用户及其所关注用户的最新微博
...
function getAuthorizationCode() {
	...
	$url = $o->getAuthorizeURL( WB_CALLBACK_URL, "code", $state );
	header("Location:$url");
	exit;
}
